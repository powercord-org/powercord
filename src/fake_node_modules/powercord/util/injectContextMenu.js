      ...
      return (props) => {
        ...
        if (!patched) {
          ...
          if (!res.type.displayName) {
            const children = wrapInHooks(res.type)(res.props);
            if (children.props.children.type.displayName === displayName) {
              patched = true;
              uninject(injectionId);

              const wrappedModule = getModule((m) => m.default === res.type || m.__powercordOriginal_default === res.type, false);
              ...
              // Make patch apply
              res.type = wrappedModule.default;
            }
