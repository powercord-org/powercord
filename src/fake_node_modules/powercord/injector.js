...
const injector = {
  ...
  inject: (injectionId, mod, funcName, patch, pre = false) => {
    ...
    if (!mod.__powercordInjectionId || !mod.__powercordInjectionId[funcName]) {
      ...
      mod[funcName] = (_oldMethod => function (...args) {
        injector._runPreInjections(id, args, this, (finalArgs) => {
  ...
  _runPreInjections: (modId, originArgs, _this, next) => {
    const injections = injector.injections.filter(i => i.module === modId && i.pre);
    injector._runPreInjectionsRecursive(injections, originArgs, _this, next);
  },

  _runPreInjectionsRecursive: (injections, originalArgs, _this, next) => {
    const injection = injections.pop();
    injection.method.call(_this, originalArgs, (args) => {
      if (injections.length > 0) {
        return injector._runPreInjectionsRecursive(injections, args, _this, next);
      }
      next();
    });
  },
