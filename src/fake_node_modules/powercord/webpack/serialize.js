...
const PTR_IN_RENDER = Symbol('powercord.serialize.pointer-render');
...
function serialize (obj, seen = new WeakMap()) {
  ...
  if (PTR_IN_RENDER in obj) {
    return {
      $$type: '$pointerRender',
      ptr: obj[PTR_IN_RENDER]
    };
  }
  ...
  while (target && target !== Object.prototype) {
    for (const key of Reflect.ownKeys(target)) {
      ...
      if ('value' in desc) {
        res[key] = serialize(target[key], seen);
      } else if ('get' in desc) {
        const fakeDesc = {
