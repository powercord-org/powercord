...
function usePointer (descriptor) {
  const { ptr } = descriptor;
...
function serialize (obj, seen = new WeakMap()) {
  ...
  if ([ 'boolean', 'string', 'number', 'bigint', 'undefined' ].includes(typeof obj) || obj === null) {
    return obj;
  }
...
const pointerProxyHandler = {
  ...
  ownKeys: (target) => {
    const props = target.__$$props || target;
    return _commandHandler('getObjectOwnKeys', props.ptr);
  },
...
const classProxyHandler = {
  ...
  has: (target, key) => _commandHandler('getObjectOwnKeys', target.__$$remotePtr).includes(key),
      ...
      if (obj.type === 'class') {
        if (!mainWorldClassMap[obj.ref]) {
          class Klass extends res {
            constructor (...args) {
              super(...args);
              const res = obj.construct(null, serialize(args));
              return Object.assign(this, deserialize(res));
            }
          }
    ...
    if (obj.type === 'class') {
      ...
      obj = class {
        constructor (...args) {
          ...
          this.__$$remoteThis = res;

          const proxy = new Proxy(this, classProxyHandler);
          let proto = Object.getPrototypeOf(this);
          while (proto) {
            for (const key of Reflect.ownKeys(proto)) {
              if ([ 'constructor', '__$$remotePrototype', '__$$remotePtr', '__$$remoteThis' ].includes(key)) {
                continue;
              }
      ...
      for (const mth of Reflect.ownKeys(prototype)) {
        ...
        if (typeof desc.value === 'function') {
          ...
          obj.prototype[mth] = function (...args) {
            const preparedArgs = serialize(args);
            const res = _commandHandler('invokeFunction', value.__$$pointer, serialize(this.__$$remoteThis || this), preparedArgs);
            return deserialize(res);
          };
