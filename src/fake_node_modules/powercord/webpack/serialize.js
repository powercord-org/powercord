...
/* global FinalizationRegistry, __$$PowercordWebpackIPC */
...
const registry = new FinalizationRegistry((ptr) => {
  ...
  if (usedPointers[ptr] === 0) {
    ...
    if (isMainWorld) {
      __$$PowercordWebpackIPC.freePointer(ptr);
    } else {
...
// Real job
// --
function shouldLeaveIntact (obj) {
  ...
  return obj === null ||
    ...
    type === 'number' ||
    type === 'bigint' ||
    (type === 'object' && (obj.toLocaleDateString || obj[Symbol.match]));
}
...
function serialize (obj, seen = new WeakMap()) {
  ...
  if (proto && proto[Symbol.toStringTag] === 'Promise') {
  ...
  if (proto && proto[Symbol.toStringTag] === 'Set') {
    const res = new Set();
    seen.set(obj, res);
  ...
  if (proto && proto[Symbol.toStringTag] === 'Map') {
    const res = new Map();
    seen.set(obj, res);
    for (const k in obj) {
      if (k in obj) {
...
const pointerProxyHandler = {
  ...
  getOwnPropertyDescriptor: (target, key) => {
    ...
    if (!props.$$cachedDescriptors) {
      props.$$cachedDescriptors = {};
    }

    if (Object.prototype.hasOwnProperty.call(props.$$cachedDescriptors, key)) {
...
function deserialize (obj, seen = new WeakMap()) {
  ...
  for (const key of Reflect.ownKeys(obj)) {
    if (key === '__$$pointerRender') {
      mainWorldPointers.set(res, obj[key]);
      usePointer(res, obj[key]);
      continue;
    }
