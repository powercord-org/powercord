...
const FUNCTION_ID = Symbol('powercord.serialize.function-id');
...
function deserialize (obj, seen = new WeakMap()) {
  ...
  if (obj.$$type === '$function') {
    if (!functionWrappers[obj.ref]) {
      functionWrappers[obj.ref] = function (...args) {
        const thisArg = serialize(this);
        const preparedArgs = args.map((arg) => serialize(arg));
        const res = obj.fn(thisArg, ...preparedArgs);
        return deserialize(res);
      };
    }

    return functionWrappers[obj.ref];
  }
