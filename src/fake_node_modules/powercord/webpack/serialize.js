...
const registry = new FinalizationRegistry((ptr) => {
  ...
  if (usedPointers[ptr] === 0) {
    ...
    queuedCleanup.push(ptr);
  }
});

setInterval(() => {
  if (isMainWorld) {
    __$$PowercordWebpackIPC.freePointers(queuedCleanup);
  } else {
    _commandHandler('releasePointers', queuedCleanup);
  }

  console.log('garbage collection: web %o, count: %d', isMainWorld, queuedCleanup.length);
  queuedCleanup = [];
}, 20e3);
...
function serialize (obj, seen = new WeakMap()) {
  ...
  if ([ 'boolean', 'string', 'number', 'bigint', 'undefined' ].includes(typeof obj) || obj instanceof Date || obj instanceof RegExp) {
    return obj;
  }
