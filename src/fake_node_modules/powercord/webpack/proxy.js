...
async function init () {
  while (!window.webpackJsonp && !window._) {
    await new Promise((resolve) => setTimeout(resolve, 1e3));
  }

  // Extract values from webpack
  webpackInstance = webpackJsonp.push([
    [],
    {
      _powercord: (_, e, r) => {
        e.cache = r.c;
  ...
  delete webpackInstance.cache._powercord;
}
...
function _getModules (filter, all) {
  ...
  if (all) {
    ...
    return exports.concat(expDefault).map((e) => serialize(e));
  }

  const exports = moduleInstances.find((m) => filter(m.exports));
  if (exports) {
    if (key) {
      cache[key] = exports.exports;
    }

    return serialize(exports.exports);
  }

  const expDefault = moduleInstances.find((m) => m.exports.default && filter(m.exports.default));
  if (expDefault) {
    if (key) {
      cache[key] = expDefault.exports.default;
    }

    return serialize(expDefault.exports.default);
  }
...
function commandHandler (cmd, ...args) {
  switch (cmd) {
    case 'getModule':
      return getModule(args[0], args[1], args[2]);
    case 'getModuleByDisplayName':
      return getModuleByDisplayName(args[0], args[1], args[2]);
    case 'getModuleByDisplayNameRaw':
      return getModuleByDisplayNameRaw(args[0], args[1], args[2], args[3]);
    case 'getAllModules':
      return _getModules(args[0], true);
    case 'getWindowProps':
...
__$$PowercordWebpackIPC.registerCommandHandler(commandHandler);
