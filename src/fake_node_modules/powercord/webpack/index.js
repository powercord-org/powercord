const { sleep } = require('powercord/util');

const moduleFilters = require('./modules.json');

exports.init = async () => {
  while (!window.webpackJsonp) {
    await sleep(1);
  }

  const moduleID = Math.random.toString();
  const instance = webpackJsonp.push([
    [],
    {
      [moduleID]: (_, e, r) => {
        e.cache = r.c;
        e.require = r;
      }
    },
    [ [ moduleID ] ]
  ]);
  delete instance.cache[moduleID];

  const getModule = (filter, retry = true, unsuccessfulIterations = 0) => {
    if (Array.isArray(filter)) {
      const keys = filter;
      filter = m => keys.every(key => m[key]);
    }

    if (unsuccessfulIterations > 16) {
      return null;
    }

    const moduleInstances = Object.values(instance.cache);
    const mdl = moduleInstances.find(m => (
      m.exports && (
        filter(m.exports) ||
        (m.exports.default && filter(m.exports.default))
      )
    ));

    if (!mdl) {
      if (!retry) {
        return null;
      }

      unsuccessfulIterations++;
      return sleep(100)
        .then(() => getModule(filter, true, unsuccessfulIterations));
    }

    return mdl.exports.default || mdl.exports;
  };

  const getAllModules = (filter) => {
    if (Array.isArray(filter)) {
      const keys = filter;
      filter = m => keys.every(key => m[key]);
    }

    const moduleInstances = Object.values(instance.cache);
    const modules = moduleInstances.filter(m => (
      m.exports && (
        filter(m.exports) ||
        (m.exports.default && filter(m.exports.default))
      )
    ));

    return modules
      .map(mdl => mdl.exports.default || mdl.exports);
  };

  const getModuleByDisplayName = (displayName) =>
    getModule(m => m.displayName && m.displayName.toLowerCase() === displayName.toLowerCase());

  const getComponentByDisplayName = (displayName) => {
    const AC = require('powercord/components/AsyncComponent');
    return (props) => getModule([ 'Component', 'PureComponent' ]).createElement(AC, {
      _provider: () => getModuleByDisplayName(displayName),
      ...props
    });
  };

  for (const mdl in moduleFilters) {
    // noinspection JSUnfilteredForInLoop
    const filters = moduleFilters[mdl];
    let target = {};

    if (filters.some(Array.isArray)) {
      for (const nestedFilters of filters) {
        Object.assign(target, await getModule(nestedFilters));
      }
    } else {
      target = await getModule(filters);
    }

    // noinspection JSUnfilteredForInLoop
    exports[mdl] = target;
  }

  delete exports.init;
  Object.assign(exports, {
    getModuleByDisplayName,
    getComponentByDisplayName,
    moduleFilters,
    getModule,
    getAllModules,
    instance
  });

  return true;
};
