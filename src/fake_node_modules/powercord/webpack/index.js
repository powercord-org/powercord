...
module.exports = class Webpack {
  ...
  static getModule (filter, retry = true) {
    ...
    if (!retry) {
      return Webpack._getModules(filter) || null;
    }
    ...
    return new Promise(async (res) => {
      ...
      res(mdl ? mdl.exports.default || mdl.exports || null : null);
    });
  }
    ...
    for (const mdl in moduleFilters) {
      ...
      if (filters.some(Array.isArray)) {
        for (const nestedFilters of filters) {
          Object.assign(target, await Webpack.getModule(nestedFilters));
        }
      } else {
        target = await Webpack.getModule(filters);
      }
  ...
  /**
   * Searches through the Webpack store
   * @param {Function} filter The filter used to get the module(s)
   * @param {Boolean} all If the function should return all modules found or not. Default to false
   * @returns {Array<object>|object} Modules found
   * @private
   */
