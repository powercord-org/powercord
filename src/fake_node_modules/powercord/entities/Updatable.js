...
class Updatable extends Events {
  ...
  async getUpdateCommits () {
    ...
    const commits = [];
    const gitLog = await exec(`git log --format="%H %an %s" ..origin/${branch}`, this.cwd)
  ...
  async update (force = false) {
    try {
      ...
      await exec(command, this.cwd).then(({ stdout }) => stdout.toString());
      return true;
    } catch (e) {
      return false;
    }
  ...
  getGitRepo () {
    return exec('git remote get-url origin', this.cwd)
      .then(r => r.stdout.toString().match(/github\.com[:/]([\w-_]+\/[\w-_]+)/)[1]);
  }

  getBranch () {
    return exec('git branch', this.cwd)
      .then(({ stdout }) =>
