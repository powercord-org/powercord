...
module.exports = class Plugin extends Updatable {
  ...
  async _unload () {
    ...
    this.registered.toasts.forEach(toast => powercord.api.toasts.unregisterToast(toast));
  ...
  registerToast ({ id, header, content, type, style, icon, imageClassName, buttons, callback, timeout }) {
    this.registered.toasts.push(id);
    powercord.api.toasts.registerToast(id, header, content, type, icon, style, imageClassName, buttons, callback, timeout);
  }
  ...
  sendToast (toast) {
    this.registered.toasts.push(toast.id);
    powercord.api.toasts.sendToast(toast);
  }

  closeToast (id) {
    powercord.api.toasts.closeToast(id);
  }
  ...
  debug (...data) {
    console.debug(`%c[Powercord:Plugin:${this.constructor.name}]`, 'color: #257dd4', ...data);
  }
