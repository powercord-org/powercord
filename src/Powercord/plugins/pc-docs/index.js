...
module.exports = class Documentation extends (
  Plugin
) {
  startPlugin() {
  ...
  async addDocsItems() {
    ...
    inject(
      ...
      (args, sections) => {
        ...
        if (changelog) {
          sections.splice(sections.indexOf(changelog) + 1, 0, {
      ...
      if (!currentChannel) {
        ...
        const channel = channels
          ...
          .find(
            (c) =>
              c.type === 0 &&
              permissions.can(discordConsts.Permissions.VIEW_CHANNEL, c)
          );
        const route = discordConsts.Routes.CHANNEL(
          channel.guild_id,
          channel.id
  ...
  async _loadModule(channel) {
    ...
    const message = module.createBotMessage(
      channel,
      '```js\nconsole.log("yeet")\n```'
    );
